...
export const PersonlInformation: React.FC<Auth.User> = (user) => {
  ...
  const handleSubmit = async () => {
    if (
      update.firstName === userInfo?.userInfo.fullName?.split(" ")[0] &&
      update.lastName === userInfo?.userInfo.fullName?.split(" ")[1] &&
      update.phoneNumber === userInfo?.userInfo.phoneNumber
    ) {
  ...
  return (
    <div className="flex-col items-start w-full lg:px-3">
      ...
      {loading ? (
        ...
        <div
          className={`grid grid-cols-2 sm:p-12 px-0 pb-24 pt-8
